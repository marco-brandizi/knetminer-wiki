...
typically you'll want to define your own settings, instead of using `--dataset-id` 
(which causes). You have to do so by creating certain files under `<dataset-dir>/settings/` (which is 
created from a codebase copy when using predefined settings). The quickest way to create such files is to 
make a copy from our [species/][67] directory and modify it.
