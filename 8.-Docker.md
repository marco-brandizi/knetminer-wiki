...
If you want to run the Knetminer container against a new dataset, you have several options:
  ...
  * Finally, you can set `KNET_DATASET_DIR` in `docker-run.sh` to point to a different `specie/` directory. 
  For instance, if you set `KNET_DATASET_DIR=/root/myinstances` and then issue `docker-run.sh new-specie`, 
  the container will look for the dataset instance files into `/root/myinstances/new-specie`. These paths 
  **refer to the container's file system**, so, in order to make this useful, you also need to map something
  like `/root/myinstances` onto some place in your host (using `KNET_DOCKER_OPTS`).
  TODO: we need to improve this, by introducing `KNET_HOST_DATASET_DIR`.
